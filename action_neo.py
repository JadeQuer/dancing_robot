...
import serial
import cv2
from rhythm_recog1 import rhythm_recog
    ...
    if not keyword:
        ...
        if keyword == "pose_recognition":
            ...
            while True:
                ...
                send_bytes = DataPackageConverter("scwreturn.bin").hex_output
                send_serial_data(ser, send_bytes)
                if pose is None:
                ...
                play_audio("resources/" + pose + ".MP3")
                send_bytes = DataPackageConverter('scw'+pose + ".bin").hex_output
                send_serial_data(ser, send_bytes)
                ...
                for action, action_data in speech_data.items():
                    ...
                    for keyword_text in action_data["list"]:
                        if keyword_text in word:
                            ...
                            audio_path='resources/'+action_keyword+'.MP3'
                            ...
                            if action in ["左移","右移","向左转","向右转","前进","后退"]:
                                ...
                                send_serial_data(ser, send_bytes,15)
                            action_found = True
